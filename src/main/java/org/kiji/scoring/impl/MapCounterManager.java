...
/** CounterManager Implementation which stores counters values in a Map. */
public final class MapCounterManager implements CounterManager {

  /**
   * Initialize a new MapCounterManager.
   *
  ...
  /** Map from counter group to Map from counter name to value. */
  private final Map<String, Map<String, Long>> mCounters = Maps.newHashMap();

  /** Prevent direct access to the constructor. Use {@link #create()}. */
  ...
  public Set<String> getCountersInGroup(
    ...
    synchronized (mCounters) {
      return mCounters.get(group).keySet();
    }
  }
}
